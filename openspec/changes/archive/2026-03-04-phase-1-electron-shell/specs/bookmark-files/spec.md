...
Bookmarks SHALL be stored as `.orqa` JSON files on disk with the schema: `{ type: "bookmark", url: string, label: string, service: string }`. Valid service values are `docs`, `sheets`, `slides`, `figma`, and `other`.

#### Scenario: Valid bookmark file
- **WHEN** the sidebar encounters a file named `metrics.orqa` containing `{"type":"bookmark","url":"https://docs.google.com/...","label":"Q2 Metrics","service":"sheets"}`
- **THEN** sidebar displays it with a green "Sheets" chip and the label "Q2 Metrics"

#### Scenario: Invalid bookmark file
- **WHEN** an `.orqa` file contains invalid JSON or missing required fields
...
`.orqa` files SHALL appear in the sidebar alongside regular files with a service-colored chip indicator: green for Sheets, blue for Docs, red for Slides, orange for Figma, grey for other.

#### Scenario: Service-colored chip
- **WHEN** sidebar renders a `.orqa` file with `service: "figma"`
- **THEN** the file node displays an orange "Figma" chip next to the label

### Requirement: Create bookmark via New Tab
Users SHALL be able to create new `.orqa` bookmark files from the New Tab screen by entering a URL, label, and selecting a service type.
...
- **THEN** system creates `checkout-flow.orqa` in the workspace root with the correct JSON and opens it as a tab
